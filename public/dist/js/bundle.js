...
angular.module('app').controller('canvasCtrl', function ($scope, mainService, $document, $compile) {
  ...
  $scope.createBox = function () {
    var template = "<svg class='draggable' width='100%' height='100%'><rect ng-mousedown='disableDrawFunc($event)' ng-mousemove='dragRect($event)' ng-click='showRectToolbar($event)' x=" + $scope.shadowXLocation + " y=" + $scope.shadowYLocation + " width=" + $scope.shadowX + " height=" + $scope.shadowY + " stroke='black' stroke-width='1' fill='white' style='opacity:0.8;cursor:move' id='dynamicId" + $scope.tempXLocation + $scope.tempYLocation + "' />  </svg>";
  ...
  $scope.createEllipse = function () {
    var template = "<svg width='100%' height='100%'><ellipse ng-mousedown='disableDrawFunc($event)' ng-mousemove='dragEllipse($event)' ng-click='showEllipseToolbar($event)' cx=" + ($scope.shadowXLocation + $scope.shadowX / 2) + " cy=" + ($scope.shadowYLocation + $scope.shadowY / 2) + " rx=" + $scope.shadowX / 2 + " ry=" + $scope.shadowY / 2 + " stroke='black' stroke-width='1' fill='white' style='opacity:0.8;cursor:move' id='dynamicId" + $scope.tempXLocation + $scope.tempYLocation + "' />    </svg>";
    var linkFn = $compile(template);
    var content = linkFn($scope);
    canvas.append(content);
    $scope.showShadow2 = false;
  ...
  $scope.createCircle = function () {
    var template = "<svg width='100%' height='100%'><circle ng-mousedown='disableDrawFunc($event)' ng-mousemove='dragCircle($event)' ng-click='showCircleToolbar($event)' cx=" + ($scope.shadowXLocation + $scope.shadowX / 2) + " cy=" + ($scope.shadowYLocation + $scope.shadowX / 2) + " r=" + $scope.shadowX / 2 + " stroke='black' stroke-width='1' fill='white' style='opacity:0.8;cursor:move' id='dynamicId" + $scope.tempXLocation + $scope.tempYLocation + "' />    </svg>";
    var linkFn = $compile(template);
    var content = linkFn($scope);
    canvas.append(content);
    $scope.showShadow2 = false;
  ...
  $scope.showRectToolbar = function (event, creationX, creationY) {
    ...
    if (!creationX) {
      ...
      var _shapeToolbarY = creationY + "px";
      var _shapeToolbarX = creationX + "px";
      $scope.shapeToolbarStyle = {
        "position": "absolute",
        "top": _shapeToolbarY,
        "left": _shapeToolbarX
